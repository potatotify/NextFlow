  ...
  return (
    <header className="pointer-events-none absolute inset-x-0 top-0 z-20">
      <div ref={menuRef} className="absolute left-4 top-4 pointer-events-auto">
        <div className="flex h-14.5 items-center rounded-xl border border-[#2b2b2b] bg-[#202020] py-2.5 pl-2.5 pr-3.5 shadow-md">
          <button
            type="button"
            onClick={() => setIsMenuOpen((value) => !value)}
            className="flex items-center gap-1.5 rounded-md px-2.5 py-2 text-[#f3f3f3] hover:bg-[#2a2a2a]"
            aria-label="Open workflow menu"
            aria-expanded={isMenuOpen}
          >
            <svg aria-label="Krea Logo" width="19" height="19" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="text-[#f4f4f4] will-change-transform">
            ...
            <ChevronDown className={`h-4 w-4 text-[#e8e8e8] transition-transform duration-150 ${isMenuOpen ? "rotate-180" : ""}`} />
          </button>
          ...
          <div className="flex items-center">
            <input
              ...
              className="h-8 min-w-[8ch] max-w-[18ch] truncate rounded-md border border-transparent bg-transparent px-2 text-base font-[450] text-[#f3f3f3] outline-none focus:border-[#313131]"
        ...
        {isMenuOpen ? (
          <div className="pointer-events-auto absolute left-0 top-full z-30 mt-2 w-64 overflow-hidden rounded-lg border border-[#262626] bg-[#1a1a1a] p-2 shadow-[0_2px_6px_0_rgba(0,0,0,0.24)]">
            <button
              type="button"
              onClick={handleImportClick}
              disabled={isImporting}
              className="flex h-10 w-full items-center gap-3 rounded-md px-3 text-left text-xs font-medium text-(--nf-text) hover:bg-[#262626]"
            >
